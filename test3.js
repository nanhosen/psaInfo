...
var lonAr = [];
var nameAr = [];
stnElevJson.map((curr)=>{
	elevAr.push([curr.wimsID,curr.elevation])
	latAr.push([curr.wimsID,curr.lat])
	lonAr.push([curr.wimsID,curr.lon])
	nameAr.push([curr.wimsID,curr.nameAr])
})

var elevArMap = new Map(elevAr)
var latArMap = new Map(latAr)
var lonArMap = new Map(lonAr)
var nameArMap = new Map(nameAr)
// console.log(latArMap)
...
//this makes the file that is rawsAllInfoElev.json. 
var percentileAr = []
f.map((curr)=>{
	percentileAr.push([curr.id,curr.name])
})

var percentileMap = new Map(percentileAr)
 ...
 psaRawsInfo.map((curr)=>{
 	// console.log(curr)
  	...
  	for(let val of stnAr){
  		...
  		var exists = percentileMap.get(valNum);
  		// console.log('e',exists,val)
  		if(exists == undefined){
  			console.log(exists, val,'curr',latArMap.get(valNum))
  			var name = gbInfo.RAWS[val];
	  			gbInfo.RAWS[val] = {
	  				"name" : name,
	  				"lat" : (latArMap.get(valNum) == undefined) ? "" : latArMap.get(valNum),
	  				"lon" : (lonArMap.get(valNum) == undefined) ? "" : lonArMap.get(valNum),
	  				"elevation" : (elevArMap.get(valNum) == undefined) ? "" : elevArMap.get(valNum),
				    "highest":"",
				    "p97": "",
				    "p90": "",
				    "p80": "",
  		...
  		f.map((curr, i)=>{
  			if(curr.id == val){
  				// console.log(elevArMap.get(curr.id))
	  			...
	  			gbInfo.RAWS[val] = {
	  				...
	  				"lat" : curr.lat,
	  				"lon" : curr.lon,
	  				"elevation" : elevArMap.get(curr.id),
				    "highest":curr.highest,
				    "p97": curr.p97,
				    "p90": curr.p90,
				    "p80": curr.p80,
